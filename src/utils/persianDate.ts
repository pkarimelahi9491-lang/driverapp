...
const JALALI_EPOCH = 2121446460; // Seconds from Unix epoch to 1 Farvardin 1348
...
export function getYearMonthKey(jalaliDate: JalaliDate): string {
  return `${jalaliDate.year}-${String(jalaliDate.month).padStart(2, '0')}`;
}
...
export function getJalaliDateTimeString(): string {
  return `${formatReadableJalali(getTodayJalali())} - ${getCurrentTimeString()}`;
}
