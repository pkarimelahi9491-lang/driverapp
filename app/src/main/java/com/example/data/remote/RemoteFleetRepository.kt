...
class RemoteFleetRepository(private val api: ApiService) {
    ...
    suspend fun login(
        ...
        try {
            ...
            if (response.isSuccessful && response.body()?.success == true) {
                ...
                Result.failure(
                    Exception(
                        response.body()?.message ?: "خطا در ورود"
                    )
                )
            }
        } catch (e: Exception) {
            Result.failure(
                Exception("ارتباط با سرور برقرار نیست")
            )
        }
    }
    ...
    suspend fun loadDrivers(
        search: String? = null
    ): Result<List<Driver>> = withContext(Dispatchers.IO) {
        try {
            ...
            if (response.isSuccessful) {
                ...
                Result.failure(
                    Exception("خطا در دریافت لیست رانندگان")
                )
            }
        } catch (e: Exception) {
            Result.failure(
    ...
    suspend fun getDriverDetail(
        id: String
    ): Result<DriverDetail> = withContext(Dispatchers.IO) {
        try {
    ...
    suspend fun loadLocations(): Result<List<LocationItem>> =
        withContext(Dispatchers.IO) {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    Result.failure(
                        Exception("خطا در دریافت مکان‌ها")
                    )
                }
            } catch (e: Exception) {
                Result.failure(
    ...
    suspend fun loadRoutes(): Result<List<RouteItem>> =
        withContext(Dispatchers.IO) {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    Result.failure(
                        Exception("خطا در دریافت مسیرها")
                    )
                }
            } catch (e: Exception) {
                Result.failure(
    ...
    ): Result<RouteItem> = withContext(Dispatchers.IO) {
        try {
            val response = api.lookupRoute(
                mapOf(
            ...
            if (response.isSuccessful && response.body()?.success == true) {
                ...
                val msg = response.body()?.message
                    ?: "این مسیر در سیستم تعریف نشده است"
    ...
    suspend fun loadTripsForDriverAndDate(
        driverId: String,
        jalaliDate: String
    ): Result<List<Trip>> = withContext(Dispatchers.IO) {
        try {
    ...
    suspend fun observeDailyWork(
        driverId: String,
        jalaliDate: String
    ): Flow<DailyWorkSummary?> = flow {
        try {
            val response = api.getDailyWork(
                driverId,
                jalaliDate
            )
    ...
    suspend fun submitDailyWork(
        ...
        try {
            val response = api.submitDailyWork(
                mapOf(
                    "driverId" to driverId,
                    "jalaliDate" to jalaliDate
                )
            )
    ...
    suspend fun loadAllDailyWorkApprovals():
        Result<List<PendingDailyApproval>> =
        withContext(Dispatchers.IO) {
            try {
                val response = api.getDailyWorks(
                    limit = "200"
                )

                if (response.isSuccessful) {
                    val dailyWorks = response.body()?.data
                        ?.map { it.toDomain() }
                        ?: emptyList()
                    ...
                    Result.failure(
                        Exception("خطا در دریافت کارکردها")
                    )
                }
            } catch (e: Exception) {
                Result.failure(
    ...
    suspend fun approveDailyWork(
        dailyWorkId: String
    ): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            val response = api.approveDailyWork(
                dailyWorkId
            )
    ...
    suspend fun rejectDailyWork(
        dailyWorkId: String,
        reason: String
    ): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            val response = api.rejectDailyWork(
                dailyWorkId,
                RejectReasonRequest(reason)
            )
    ...
    suspend fun unlockDailyWork(
        dailyWorkId: String
    ): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            val response = api.unlockDailyWork(
                dailyWorkId
            )
    ...
    suspend fun getMonthlySettlement(
        yearMonth: String
    ): Result<List<MonthlySettlementRow>> =
        withContext(Dispatchers.IO) {
            try {
                val response = api.getMonthlyReport(
                    yearMonth
                )
                ...
                if (response.isSuccessful) {
                    ...
                    Result.failure(
                        Exception("خطا در دریافت گزارش مالی")
                    )
                }
            } catch (e: Exception) {
                Result.failure(
    ...
    suspend fun loadAuditLogs():
        Result<List<com.example.data.local.entity.AuditLogEntity>> =
        withContext(Dispatchers.IO) {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    Result.failure(
                        Exception("خطا در دریافت لاگ‌ها")
                    )
                }
            } catch (e: Exception) {
                Result.failure(
    ...
    ): Result<Driver> = withContext(Dispatchers.IO) {
        try {
    ...
    suspend fun toggleDriverStatus(
        driverId: String
    ): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            val response = api.toggleDriver(
                driverId
            )
